...
type Historico = { mes: string; aberta: number; agendada: number; em_andamento: number; concluida: number; cancelada: number }

const CARDS = [
  { key: 'aberta',       label: 'Abertas',       icon: ClipboardList,  bg: 'bg-action',       text: 'text-white'   },
  { key: 'agendada',     label: 'Agendadas',     icon: Clock,          bg: 'bg-scheduled',    text: 'text-primary' },
  { key: 'em_andamento', label: 'Em Andamento',  icon: AlertTriangle,  bg: 'bg-warning',      text: 'text-primary' },
  { key: 'concluida',    label: 'Concluídas',    icon: CheckCircle,    bg: 'bg-action',       text: 'text-white'   },
  { key: 'cancelada',    label: 'Canceladas',    icon: XCircle,        bg: 'bg-destructive',  text: 'text-white'   },
]

export default async function DashboardPage() {
  const [painel, historico] = await Promise.all([
    api.get<Painel>('/ordens-servico/painel').catch(() => ({} as Painel)),
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 mb-6">
        {CARDS.map(({ key, label, icon: Icon, bg, text }) => (
          ...
          >
            ...
            <p className={`text-3xl font-bold ${text}`}>{painel[key] ?? 0}</p>
      ...
      <a
        href="/ordens-servico"
        className="bg-white rounded-2xl p-6 shadow-sm border border-border flex items-center justify-between hover:bg-surface transition-colors block mb-6"
      >
        <div>
