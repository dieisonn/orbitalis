...
export function PrintPMOC({ plano, config }: { plano: Plano; config: Config }) {
  ...
  // Nº: 5mm | Descrição: 80mm | meses: restante dividido
  ...
  return (
    <>
      ...
      <div style={{ pageBreakAfter: 'always', minHeight: '260mm', display: 'flex', flexDirection: 'column', fontFamily: 'Arial, sans-serif' }}>
        ...
        {/* Faixa superior com logo e dados da empresa */}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 24px', textAlign: 'center', backgroundColor: '#fafbff' }}>
          ...
          <p style={{ fontSize: '11pt', fontWeight: 700, color: '#374151', textTransform: 'uppercase', letterSpacing: '0.12em', lineHeight: 1.6 }}>
            Plano de Manutenção,
          </p>
          <p style={{ fontSize: '11pt', fontWeight: 700, color: '#374151', textTransform: 'uppercase', letterSpacing: '0.12em', lineHeight: 1.6 }}>
            Operação e Controle
          ...
          <p style={{ fontSize: '38pt', fontWeight: 900, color: cor, letterSpacing: '0.25em', margin: '8px 0 4px' }}>
            PMOC
          </p>
        ...
        <div style={{ padding: '0 24px 20px' }}>
          ...
          {/* Grade 2 col: Contratante | Imóvel */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>

            <div style={S.box(cor)}>
              <div style={S.sectionHeader(cor)}>
                <p style={S.label(cor)}>Contratante</p>
              </div>
              <div style={S.sectionBody}>
                <p style={{ fontWeight: 700, fontSize: '9pt', marginBottom: '3px' }}>{plano.ambiente.cliente.razaoSocial}</p>
                {plano.ambiente.cliente.nomeFantasia && (
                  <p style={{ fontSize: '8pt', color: '#6b7280', marginBottom: '3px' }}>{plano.ambiente.cliente.nomeFantasia}</p>
                )}
                <p style={{ fontSize: '7.5pt', color: '#6b7280' }}>CNPJ/CPF: {plano.ambiente.cliente.documento}</p>
                {plano.ambiente.cliente.telefone && (
                  <p style={{ fontSize: '7.5pt', color: '#6b7280' }}>Tel: {plano.ambiente.cliente.telefone}</p>
                )}
                <p style={{ fontSize: '7.5pt', color: '#6b7280', marginTop: '2px' }}>{plano.ambiente.cliente.endereco}</p>
              </div>
            </div>

            <div style={S.box(cor)}>
              <div style={S.sectionHeader(cor)}>
                <p style={S.label(cor)}>Imóvel / Ambiente Climatizado</p>
              </div>
              <div style={S.sectionBody}>
                <p style={{ fontWeight: 700, fontSize: '9pt', marginBottom: '3px' }}>{plano.ambiente.nome}</p>
                <p style={{ fontSize: '7.5pt', color: '#6b7280' }}>{plano.ambiente.localizacaoInterna}</p>
                <p style={{ fontSize: '7.5pt', color: '#6b7280', marginTop: '3px' }}>
                  Área: {plano.ambiente.metrosQuadrados} m² · Cap. Térmica: {plano.ambiente.capacidadeTermica}
                </p>
                <p style={{ fontSize: '7.5pt', color: '#6b7280', marginTop: '2px' }}>
                  {plano.equipamentos.length} equipamento{plano.equipamentos.length !== 1 ? 's' : ''} cadastrado{plano.equipamentos.length !== 1 ? 's' : ''}
                </p>
              </div>
            </div>
          </div>

          {/* Grade 2 col: Responsável Técnico | Vigência */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>

            <div style={S.box(cor)}>
              <div style={S.sectionHeader(cor)}>
                <p style={S.label(cor)}>Responsável Técnico pela Elaboração</p>
              </div>
              <div style={S.sectionBody}>
            ...
            <div style={S.box(cor)}>
              <div style={S.sectionHeader(cor)}>
                <p style={S.label(cor)}>Vigência do PMOC</p>
              </div>
              <div style={S.sectionBody}>
                ...
                <p style={{ fontSize: '7.5pt', color: '#6b7280' }}>
                  Periodicidade: <strong>{freq}</strong>
                </p>
                <p style={{ fontSize: '7.5pt', color: '#6b7280', marginTop: '2px' }}>
                  Ciclos previstos: <strong>{plano.ordensServico.length}</strong>
                </p>
              </div>
            </div>
          </div>
        ...
        <div style={{ backgroundColor: cor + '10', borderTop: `1px solid ${cor}20`, padding: '6px 24px', display: 'flex', justifyContent: 'space-between' }}>
          <p style={{ fontSize: '6.5pt', color: '#9ca3af' }}>
            {nomeEmpresa}{config?.cnpj ? ` · CNPJ ${config.cnpj}` : ''}
          </p>
          <p style={{ fontSize: '6.5pt', color: '#9ca3af' }}>
            PMOC Nº {plano.id.slice(0, 8).toUpperCase()} · Pág. 1
          </p>
        </div>
      </div>

      {/* ═══════════════════════════════════════════════════════════
          PÁGINA 2 — IDENTIFICAÇÃO + TABELA DE EQUIPAMENTOS
      ═══════════════════════════════════════════════════════════ */}
      <div className="page-break" style={{ fontFamily: 'Arial, sans-serif' }}>
        ...
        <div style={{ padding: '12px 20px 10px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
            <div style={S.box(cor)}>
              <div style={S.sectionHeader(cor)}><p style={S.label(cor)}>Imóvel / Ambiente</p></div>
              <div style={{ padding: '8px 12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                ...
                <InfoRow label="Localização Interna" value={plano.ambiente.localizacaoInterna} full />
            ...
            <div style={S.box(cor)}>
              ...
              <div style={{ padding: '8px 12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                ...
                {plano.ambiente.cliente.nomeFantasia && <InfoRow label="Nome Fantasia" value={plano.ambiente.cliente.nomeFantasia} full />}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
            ...
            <div style={S.box(cor)}>
              ...
              <div style={{ padding: '8px 12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                ...
                <InfoRow label="Ciclos Previstos" value={String(plano.ordensServico.length)} />
              </div>
            </div>
          </div>

          {/* Tabela de equipamentos */}
          <div style={{ ...S.box(cor), marginBottom: '10px' }}>
            <div style={S.sectionHeader(cor)}>
              <p style={S.label(cor)}>Ambientes Climatizados e Equipamentos</p>
            </div>
              ...
              <tbody>
                {plano.equipamentos.map((eq, idx) => (
                  <tr key={eq.id} style={{ borderBottom: '1px solid #f3f4f6', backgroundColor: idx % 2 === 1 ? '#fafafa' : 'white' }}>
                    ...
                    <td style={{ padding: '5px 8px', color: '#6b7280' }}>{[eq.marca, eq.modelo].filter(Boolean).join(' ') || '—'}</td>
                    <td style={{ padding: '5px 8px', fontFamily: 'monospace', fontSize: '7.5pt', color: '#6b7280' }}>{eq.numeroSerie || '—'}</td>
                    <td style={{ padding: '5px 8px', color: '#6b7280' }}>{plano.ambiente.nome} · {plano.ambiente.localizacaoInterna}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {plano.equipamentos.map((eq, eqIdx) => (
        <div key={eq.id} className="page-break" style={{ fontFamily: 'Arial, sans-serif', pageBreakBefore: 'always' }}>

          <MiniHeader
            cor={cor}
            nomeEmpresa={nomeEmpresa}
            logoUrl={logoUrl}
            titulo={`Checklist de Manutenção — ${eq.nome}`}
            sub={`${[eq.marca, eq.modelo].filter(Boolean).join(' ')}${eq.numeroSerie ? ` · S/N: ${eq.numeroSerie}` : ''} · ${eq.tipoEquipamento}`}
            right={`${clienteLabel} · ${freq} · ${fmtDate(dataInicioPMOC)} – ${fmtDate(plano.dataFim)} · Equip. ${eqIdx + 1}/${plano.equipamentos.length}`}
          />
          ...
          <div style={{ padding: '8px 20px 10px' }}>
            {itensChecklist.length === 0 ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', fontSize: '7pt' }}>
                ...
                <thead>
                  <tr>
                    <th style={{ padding: '4px 2px', textAlign: 'center', fontSize: '6pt', fontWeight: 700, color: '#9ca3af', border: '1px solid #e5e7eb', backgroundColor: '#f9fafb' }}>
                      Nº
                    </th>
                  ...
                  {itensChecklist.map((item, idx) => (
                    <tr key={item.id} style={{ backgroundColor: idx % 2 === 1 ? '#fafafa' : 'white' }}>
                      ...
                      <td style={{ padding: '3px 6px', border: '1px solid #e5e7eb', lineHeight: 1.3 }}>
                        <span style={{ fontSize: '7pt', fontWeight: item.obrigatorio ? 600 : 400 }}>
                          {item.descricao}
                        </span>
            ...
            <div style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div style={{ border: '1px solid #e5e7eb', borderRadius: '4px', padding: '8px' }}>
                <p style={{ fontSize: '6.5pt', fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>
...
function InfoRow({ label, value, full }: { label: string; value: string; full?: boolean }) {
  return (
    <div style={{ gridColumn: full ? '1 / -1' : undefined }}>
      <p style={{ fontSize: '6pt', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '1px' }}>
        {label}
      </p>
